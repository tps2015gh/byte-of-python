...
Functions เป็นส่วนของโปรแกรมที่ใช้ซ้ำได้. มันยอมให้คุณกำหนดชื่อให้กับ block ของ statements, ยอมให้คุณรันโค้ด block นั้นโดยใช้ชื่อที่กำหนดไว้ในที่ใดๆก็ตามในโปรแกรมของคุณ และใช้ซ้ำได้โดยไม่จำกัดจำนวนครั้ง. สิ่งนี้เป็นที่รู้จักกันว่า *calling* (*การเรียกใช้*) ฟังก์ชัน. พวกเราได้เคยเรียกใช้ build-in function กันมาแล้วเช่น  `len` และ `range`.
...
We will explore tuples and dictionaries in a [later chapter](./12.data_structures.md#data-structures).
...
A string on the first logical line of a function is the *docstring* for that function. Note that DocStrings also apply to [modules](./11.modules.md#modules) and [classes](./14.oop.md#oop) which we will learn about in the respective chapters.
...
We can access the docstring of the `print_max` function using the `__doc__` (notice the *double underscores*) attribute (name belonging to) of the function. Just remember that Python treats *everything* as an object and this includes functions. We'll learn more about objects in the chapter on [classes](./14.oop.md#oop).
